...
%type <DECLARATOR_LIST> declaration_list
...
struct_specifier 
        ...
        '{'
         ...
         declaration_list '}' ';'	{ string mod_name= $2 + "#"; globalSymbolTable[mod_name]->sz=offset; }
        ;
...
primary_expression 
        ...
        | STRING_LITERAL { $$ = new StringConst($1); $$ -> type = "string";printstr.push_back($1);}
        | '(' expression ')' { $$ = $2; }
        ;
	...
	| '{' statement_list '}' 	{ globalSymbolTable[current_func]->ast = $2;}
        ...
        declaration_list 
        {
            for(int i = 0 ; i < ($3).size() ; i++)
                modify_offset_of_locals(current_func, ($3)[i], offset);
        }
        statement_list '}' {	globalSymbolTable[current_func]->ast = $5;}
	;
...
statement
        ...
        if(!isParameter && $2 -> type.find(',') != string :: npos) {
        cout<<"ERROR at line: " << line_count << " : cannot return address of a local variable\n"; err=true; ABORT();
        }
...
postfix_expression
	: primary_expression  		{ $$ = (UnaryExpAst*) $1;} 	
        ...
        {
            if(!($1->is_lvalue) && !($1->astnode_type=="funcall")){
        ...
        {   
            ...
            if(t != "-") {
                ...
                if(t.find('#') != string::npos)
                    t = t.substr(0, t.length() - 1);
                cout << "ERROR at line: " << line_count << " Struct '" << t << "' has no attribute named '" << $3 << "'\n";
                err=true; ABORT();
            }
        }
        ...
        { string t = $1->type; $$ = new UnaryExpAst($1, "INC_OP"); $$->type=t; } // Cannot appear on the LHS of '='   Enforce this
